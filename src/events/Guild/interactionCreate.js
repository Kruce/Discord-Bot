                ...
                if (cooldown.has(cooldownKey)) {
                    ...
                    if (data.some((v) => v === interaction.commandName)) {
                        const cooldownMessage = (isGlobalCooldown
                            ? config.messageSettings.globalCooldownMessage ?? "please wait before using this command. global cooldown: ({cooldown}s)."
                            : config.messageSettings.cooldownMessage ?? "please wait before using this command. cooldown: ({cooldown}s).").replace(/{cooldown}/g, command.options.cooldown / 1000);
